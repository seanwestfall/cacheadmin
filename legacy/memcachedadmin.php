...
<head>
	...
	<!-- react -->
        <script src="memcachedadmin/bower_components/react/react.js"></script>
	<script src="https://cdnjs.cloudflare.com/ajax/libs/react/0.13.3/JSXTransformer.js"></script>
        <script src="memcachedadmin/bower_components/ngReact/ngReact.js"></script>
        <!--<script src="memcachedadmin/app.js"></script>-->

	<!-- underscore -->
	<script src='https://cdnjs.cloudflare.com/ajax/libs/underscore.js/1.8.3/underscore.js'></script>

	<link href='memcachedadmin/metricsgraphics.css' rel='stylesheet' type='text/css'>
	<script src='memcachedadmin/metricsgraphics.min.js'></script>
	<link href='memcachedadmin/tipsy.css' rel='stylesheet' type='text/css'>
	<script src='memcachedadmin/jquery.tipsy.js'></script>
...
<body ng-controller="mainCtrl">
	<div class="head">
		<img src="internetbrands.png"></img>
	</div>
	<div class="content">
	...
	<table class="table table-striped">
		...
		<tr ng-repeat="(key, value) in slabdata" ng-hide="value.name.substring(0,4)!='STAT'">
			<td><button type="button" class="btn btn-success" data-ng-click="getDump(value.id_num)">Dump</button></td>
			<td>{{value.id_num}}</td>
	...
	<h2>Table2</h2>
	<div id="react2">
	<slab-table-test table="table" />
	</div>
	<button type="button" class="btn btn-primary" ng-click="regenerate()">generate</button>
	<div class="charts">
	...
	<table class="table">
		...
		<tr ng-repeat-end ng-show="value.html">
			...
			<button type="button" class="btn btn-danger floatright" ng-click="deleteKey(key)">Delete From Cache</button>
			</div>
			<td>
		</tr>
	</table>
	...
	<script>
		var memcachedApp = angular.module('memcachedApp', ['react']);
		...
		memcachedApp.controller('mainCtrl', function($scope, $http, $sce) {
			...
			$scope.generateRows = function() {
				$http.get('memcachedadmin/getslabdata.php').then(function(data) {
					...
					for(var key in data.data) {
						...
						if (!slabdata[arrItem[0]]) {
							slabdata[arrItem[0]] = {};
						}
						if(arrItem[1]) {
							var fields = arrItem[1].split(" ");
							slabdata[arrItem[0]][fields[0]] = Number(fields[1]);
							slabdata[arrItem[0]]['name'] = arrItem[0];
							slabdata[arrItem[0]]['id_num'] = arrItem[0].substring(5);
						}
					}
					var rows = [];
					for(var key in slabdata) {
						if(slabdata[key].id_num) {
							rows.push( [
								slabdata[key].id_num, // simply passed to the ng-click directive
								slabdata[key].id_num,
								slabdata[key].chunk_size,
								slabdata[key].chunks_per_page,
								slabdata[key].get_hits,
								slabdata[key].mem_requested,
								slabdata[key].total_chunks,
								slabdata[key].total_pages,
								slabdata[key].used_chunks
							] );
						}
					}
					// update angular
					...
					var thistable = {
			    		...
			    		//React.render(React.createElement(SlabTable, _.extend({}, this.props, { table: thistable, type: 'button', class: 'btn btn-success', ngclick: 'getText()', text: 'Dump' })),document.getElementById('react'));
				});
			}
			...
			$scope.generateRows();
			$scope.table = {
			    cols: [ '', 'chuck_size', 'chunks_per_page', 'get_hits', 'mem_requested', 'total_chunks', 'total_pages', 'used_chunks' ],
			    rows: getRows()
			};

			$scope.regenerate = function() {
			    $scope.generateRows();
			    $scope.table.rows = getRows();
			}
			...
			$scope.searchKey = function() {
				$scope.currentSlabDumpId = "Search for Keyword";
				$scope.slabdump = {};
				var term = $scope.key + "+";
				var re = new RegExp(term);
				for(key in $scope.slabdata) {
					var idnum = key.substring(5);
					if(!isNaN(idnum)) {
						$http.get('memcachedadmin/dumpslab.php?slabid=' + idnum).then(function(data) {
							for(var key in data.data) {
								...
								if(re.exec(arrItem[1])) {
									...
									var len = sndIndex - fstIndex;
									var arrStat = data.data[key].substring(fstIndex+1,sndIndex).split(';');
									$scope.slabdump[key].size = arrStat[0]; // size of key
									$scope.slabdump[key].time = arrStat[1]; // expiration of key
								}
							}
						});
					}
			...
			$scope.deleteKey = function(key) {
				$http.get('memcachedadmin/deleteKey.php?key=' + key).success(function() {
					delete $scope.slabdump[key];
				});
			}
