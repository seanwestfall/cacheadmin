...
var App = angular.module('singular', ['ngRoute', 'ngAnimate', 'ngStorage', 'ngCookies', 'pascalprecht.translate', 'ui.bootstrap', 'ui.router', 'oc.lazyLoad', 'cfp.loadingBar', 'ui.utils'])
    ...
    // ----------------------------------- 
		...
		$rootScope.selectedip = "127.0.0.1";
		$rootScope.selectedport = "11211";
    $rootScope.app = {
...
App.config(['$stateProvider','$urlRouterProvider', '$controllerProvider', '$compileProvider', '$filterProvider', '$provide', '$ocLazyLoadProvider', 'appDependencies',
    function ($stateProvider, $urlRouterProvider, $controllerProvider, $compileProvider, $filterProvider, $provide, $ocLazyLoadProvider, appDependencies) {
      ...
      $urlRouterProvider.otherwise('/app/dashboard-table');
      ...
      $stateProvider
        ...
        .state('app.table-ngtable-dashboard', {
            url: '/dashboard-table',
            templateUrl: basepath('table-ngtable-ib.html'),
            controller: 'NullController',
            resolve: requireDeps('ngTable', 'ngTableExport', 'flot-chart','flot-chart-plugins')
        })
        .state('app.table-ngtable-slab', {
            url: '/slab/:slabid',
            templateUrl: basepath('table-ngtable-ib-slab.html'),
            controller: 'NullController',
            resolve: requireDeps('ngTable', 'ngTableExport')
        })
        .state('app.table-ngtable-search', {
            url: '/search',
            templateUrl: basepath('table-ngtable-search.html'),
            controller: 'NullController',
            resolve: requireDeps('ngTable', 'ngTableExport')
        })
        .state('app.table-ngtable-settings', {
            url: '/settings',
            templateUrl: basepath('table-ngtable-settings.html'),
            controller: 'NullController',
...
App.controller('HeaderNavController', ['$scope', '$rootScope', function($scope, $rootScope) {
  ...
  $scope.headerTitle = $rootScope.selected;
...
App.controller('SidebarController', ['$rootScope', '$scope', '$location', '$http', '$timeout', 'sidebarMemu', 'appMediaquery', '$window',
  function($rootScope, $scope, $location, $http, $timeout, sidebarMemu, appMediaquery, $window ){
    ...
    $rootScope.select = function(index, item) {
	if(item.selectsubnav) {
	        var selectIndex = $rootScope.menuItems.map(function(e) { return e.isselect }).indexOf(true);
		$rootScope.menuItems[selectIndex].text = item.text;
		$rootScope.selected = item.text;
		$rootScope.selectedip = item.ip;
		$rootScope.selectedport = item.port;
	}
    }
...
App.service('sidebarMemu', ["$rootScope", "$http", "$q", function($rootScope, $http, $q) {
  'use strict';
  ...
  return {
    load: function() {
      ...
      $http.get(menuURL)
	   ...
	   var slabsIndex = items.map(function(e) { return e.text }).indexOf("Slabs");
	   var selectIndex = items.map(function(e) { return e.isselect }).indexOf(true);

	var defer = $q.all([$http.get('app/php/getslabdata.php?ip=' + $rootScope.selectedip + '&port=' + $rootScope.selectedport), $http.get('data/instances.json')]);
	...
	defer.then(function(data) {
		...
		for(var key in data[0].data) {
			...
			var item = {};
			var arrItem = data[0].data[key].split(":");
			if (!$rootScope.slabdata[arrItem[0]]) {
				$rootScope.slabdata[arrItem[0]] = {};
			}
			if(arrItem[1]) {
				var fields = arrItem[1].split(" ");
				$rootScope.slabdata[arrItem[0]][fields[0]] = Number(fields[1]);
				$rootScope.slabdata[arrItem[0]]['name'] = arrItem[0];
				$rootScope.slabdata[arrItem[0]]['id_num'] = arrItem[0].substring(5);
			}
		}
		
		for (var n in $rootScope.slabdata) {
		   var slab = $rootScope.slabdata[n];
		   items[slabsIndex].subnav.push({
		     sref: 'app.table-ngtable-slab({slabid: '+slab.id_num+'})',
		     text: slab.name, 
		     alert: {"text":slab.used_chunks, "class":"label-primary"},
		     translate: null
		   });	
		};

	   	$rootScope.selectJson = JSON.parse(data[1].data.instances);
		$rootScope.selectJson.forEach(function(n) {
			items[selectIndex].subnav.push({
				sref: '#',
				selectsubnav: true,
				text: n.title,
				default: n.default,
			...
			if(n.default) {
				items[selectIndex].text = n.title;
				$rootScope.selected = n.title;
				$rootScope.selectedip = n.ip;
				$rootScope.selectedport = n.port;
		...
		items[selectIndex].subnav.push({
		    "text": "Add",
		    "sref" : "app.table-ngtable-settings",
		    "icon": "icon-plus",
		    "translate": null
  		});
...
App.controller('AngularSlabTableIBController', AngularSlabTableIBController);
function AngularSlabTableIBController($scope, $filter, ngTableParams, $http, $stateParams, $sce, $rootScope, flotOptions) {
  'use strict';
  var vm = this;

       var id = $stateParams.slabid;
		...
		for(var key in data.data) {
			...
			if(!bSpecial) {
				...
				if(!vm.averageKeys[keyTemp]){
					...
					vm.averageKeys[keyTemp].counter = 0;
					vm.averageKeys[keyTemp].total = 0;
			...
			vm.averageKeys[keyTemp].counter++;
			// stats index
			...
			vm.slabdump[key].size = arrStat[0];
			vm.slabdump[key].time = arrStat[1];  

			vm.averageKeys[keyTemp].total += Number(arrStat[0].replace(/[b]/,'').trim());
		}

		for(var key in vm.averageKeys) {
			vm.totalOcc += vm.averageKeys[key].counter;
			vm.totalSize += vm.averageKeys[key].total;
		}
	});
	  vm.tableParams = new ngTableParams({
              page: 1,            // show first page
              count: vm.averageKeys.length,
	      sorting: {
		  name: 'desc'     // initial sorting
	      }
	  }, {
              counts: [],
	      total: vm.averageKeys.length, // length of data
	      getData: function($defer, params) {
		  // use build-in angular filter
		  var orderedData = params.sorting() ?
			  $filter('orderBy')(vm.averageKeys, params.orderBy()) :
			  vm.averageKeys;
	...
	vm.getValueFromDump = function(index) {
                  if(vm.slabdump[index].html) {
				vm.slabdump[index].html = null;
			} else {
				$http.get('app/php/getValue.php?ip=' + $rootScope.selectedip + '&port=' + $rootScope.selectedport + '&key=' + vm.slabdump[index].key).then(function(data) {
					vm.slabdump[index].html = $sce.trustAsHtml(data.data);
				});
		  }
	}
...
AngularSlabTableIBController.$inject = ["$scope", "$filter", "ngTableParams", "$http", "$stateParams","$sce","$rootScope", "flotOptions"];


App.controller('AngularTableIBController', AngularTableIBController);
function AngularTableIBController($scope, $filter, ngTableParams, $http, $rootScope) {
  'use strict';
  var vm = this;

  $http.get('app/php/getmemcacheddata.php?ip=' + $rootScope.selectedip + '&port=' + $rootScope.selectedport).then(function(data) {
      vm.statsdata = data.data['localhost:11211'];
  });
  $http.get('app/php/getslabdata.php?ip=' + $rootScope.selectedip + '&port=' + $rootScope.selectedport).then(function(data) {
      vm.slabdata = {};
	for(var key in data.data) {
		var item = {};
		var arrItem = data.data[key].split(":");
		if (!vm.slabdata[arrItem[0]]) {
			vm.slabdata[arrItem[0]] = {};
		}
		if(arrItem[1]) {
			var fields = arrItem[1].split(" ");
			vm.slabdata[arrItem[0]][fields[0]] = Number(fields[1]);
			vm.slabdata[arrItem[0]]['name'] = arrItem[0];
			vm.slabdata[arrItem[0]]['id_num'] = arrItem[0].substring(5);
		}
	}

	  vm.tableParams = new ngTableParams({
              page: 1,            // show first page
              count: vm.slabdata.length,
	      sorting: {
		  name: 'desc'     // initial sorting
	      }
	  }, {
              counts: [],
	      total: vm.slabdata.length, // length of data
	      getData: function($defer, params) {
		  // use build-in angular filter
		  var orderedData = params.sorting() ?
			  $filter('orderBy')(vm.slabdata, params.orderBy()) :
			  vm.slabdata;
...
AngularTableIBController.$inject = ["$scope", "$filter", "ngTableParams", "$http", "$rootScope"];

App.controller('AngularTableIBController', AngularTableIBController);
function AngularTableIBController($scope, $filter, ngTableParams, $http, flotOptions, colors, $rootScope, $interval) {
  'use strict';
  var vm = this;
  ...
  $http.get('app/php/getmemcacheddata.php?ip=' + $rootScope.selectedip + '&port=' + $rootScope.selectedport).then(function(data) {
	console.log(data);
      vm.statsdata = data.data[$rootScope.selectedip+':'+$rootScope.selectedport];

      vm.memory = [
	{"color" : "#fff",
         "data" : (vm.statsdata['limit_maxbytes']-vm.statsdata['bytes'])/vm.statsdata['limit_maxbytes']*100,
         "label" : "Free Memory"
        },
        { "color" : "#00b4ff",
          "data" : vm.statsdata['bytes']/vm.statsdata['limit_maxbytes']*100,
          "label" : "Used Memory" 
        }
      ];
      ...
      hit = {"label": "hits",
             "color": "#ec008c",
             "data": [["hits", Math.floor(vm.statsdata['get_hits']/vm.statsdata['cmd_get']*100)]]};
      var miss = {};
      miss = {"label": "misses",
              "color": "#937fc7",
              "data": [["misses", Math.floor(vm.statsdata['get_misses']/vm.statsdata['cmd_get']*100)]]};
      vm.bar = [hit, miss];
  ...
  var totalKeys = 0;
	for(var key in data.data) {
		...
		if(arrItem[1]) {
			...
			if(fields[0]=="used_chunks") {
				totalKeys += Number(fields[1]);
			}
		}
	...
	var step_num = 1;
	var slabsLen = Object.keys(vm.slabdata).length;
	vm.flotData = [];
	vm.flotData2 = [];
	for(var slab in vm.slabdata) {
	  var currSlab = vm.slabdata[slab];
   	  if(currSlab) {
		  var data = {};
		  var data2 = {};
		  data.data = currSlab.used_chunks / totalKeys;
		  data.label = currSlab.name;
		  data.color = rainbow(slabsLen, step_num);

		 data2.data = (currSlab.used_chunks * currSlab.chunk_size);
		 data2.label = currSlab.name;
		 data2.color = rainbow(slabsLen, step_num);
		  vm.flotData.push(data);
		  vm.flotData2.push(data2);
		  step_num++;
	  }
	}
  })}, 1000); 
...
AngularTableIBController.$inject = ["$scope", "$filter", "ngTableParams", "$http", "flotOptions", "colors", "$rootScope", "$interval"];

App.controller('AngularTableSearchController', AngularTableSearchController);
function AngularTableSearchController($scope, $filter, ngTableParams, $http, flotOptions, colors, $rootScope, $sce) {
  'use strict';
  var vm = this;
...
vm.html = null;
  ...
  vm.searchKey = function() {
	...
	for(var key in $rootScope.slabdata) {
		var idnum = key.substring(5);
		if(!isNaN(idnum)) {
			$http.get('app/php/dumpslab.php?ip=' + $rootScope.selectedip + '&port=' + $rootScope.selectedport + '&slabid=' + idnum).then(function(data) {
				for(var key in data.data) {
					var arrItem = data.data[key].split(" "); // set key
					if(re.exec(arrItem[1])) {
						if (!vm.slabdump[key]) {
							vm.slabdump[key] = {};
						}
						vm.slabdump[key]['key'] = arrItem[1];
						...
						var len = sndIndex - fstIndex;
						var arrStat = data.data[key].substring(fstIndex+1,sndIndex).split(';');
						vm.slabdump[key].size = arrStat[0]; // size of key
						vm.slabdump[key].time = arrStat[1]; // expiration of key
						vm.count++;
					}
				}
			});
	  ...
	  vm.tableParams = new ngTableParams({
              page: 1,            // show first page
              count: vm.slabdump.length,
	      sorting: {
		  name: 'desc'     // initial sorting
	      }
	  }, {
              counts: [],
	      total: vm.slabdump.length, // length of data
	      getData: function($defer, params) {
		  // use build-in angular filter
		  var orderedData = params.sorting() ?
			  $filter('orderBy')(vm.slabdump, params.orderBy()) :
			  vm.slabdump;
...
App.controller('AngularTableSettingsController', AngularTableSettingsController);
function AngularTableSettingsController($scope, $filter, ngTableParams, $http, flotOptions, colors, $rootScope, $sce, toaster) {
	  ...
	  $http.get('data/'+$rootScope.selectedip+'_rules.json').then(function(data) {
		vm.rulesJson = JSON.parse(data.data.rules);
          ...
          vm.selectJson = $rootScope.selectJson;
	  vm.tableParams = new ngTableParams({
              page: 1,            // show first page
              count: vm.selectJson.length,
	      sorting: {
		  name: 'desc'     // initial sorting
	      }
	  }, {
              counts: [],
	      total: vm.selectJson.length, // length of data
	      getData: function($defer, params) {
		  // use build-in angular filter
		  var orderedData = params.sorting() ?
			  $filter('orderBy')(vm.selectJson, params.orderBy()) :
			  vm.selectJson;
	...
	vm.setDefault = function($index) {
		...
		vm.selectJson.forEach(function(n) {
			n.default = false;
		});

		$rootScope.selectJson[$index].default = true;
		vm.selectJson[$index].default = true;
		console.log($rootScope.selectJson);
	}
	...
	vm.delete = function($index) {
		vm.selectJson.splice($index,1);
		$rootScope.selectJson.splice($index,1);
	}
	vm.delete2 = function($index) {
		vm.rulesJson.splice($index,1);
		$rootScope.selectJson.splice($index,1);
	}
	...
	vm.saveChanges = function() {
		$http.post('app/php/saveinstances.php', {instances: JSON.stringify($rootScope.selectJson) }).
		then(function(response) {
			// save changes to the sidebar

				var items = $rootScope.menuItems;
				   var selectIndex = items.map(function(e) { return e.isselect }).indexOf(true);


			items[selectIndex].subnav = [];
			  ...
			  $scope.toaster = {
			    ...
			    toaster.pop('success', 'Success', 'Memcached Instances Changes Saved!');
		});
	}
	vm.saveChanges2 = function() {
		$http.post('app/php/saverules.php', {instance: $rootScope.selectedip, rules: JSON.stringify(vm.rulesJson) }).
		then(function(response) {
			// toast
			toaster.pop('success', 'Success', 'Rules Saved!');
		});
...
AngularTableSettingsController.$inject = ["$scope", "$filter", "ngTableParams", "$http", "flotOptions", "colors", "$rootScope", "$sce", "toaster"];
